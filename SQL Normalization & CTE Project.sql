...
-- CTE Basics:
-- Q.5 Write a query using a CTE to retrieve the distinct list of actor names and the number of films they have -- acted in from the and tables
with actorfilm_count as( select a.actor_id, (concat( first_name,' ', last_name)) as actor_name, 
count(film_id) as number_of_film from actor as a join film_actor as fc on a.actor_id = fc.actor_id 
group by actor_id,actor_name ) select actor_name, number_of_film from actorfilm_count 
...
WITH RECURSIVE CategoryHierarchy AS (
    SELECT category_id, name AS category_name, Null as parent_category_id, 0 as depth
    FROM category
	WHERE parent_id IS NULL
UNION ALL
    SELECT c.category_id, c.category_name, c.parent_id, ch.level + 1 AS level
    FROM category c
...
-- CTE with Joins:
-- Q.7 Create a CTE that combines information from the film and languge tables to display the film title, language -- name, and rental rate
...
-- CTE for Aggregation:
-- Q.8 Write a query using a CTE to find the total revenue generated by each customer (sum of payments) from -- the customer and payments tables
WITH totalcustomer_revenue AS (SELECT c.customer_id, CONCAT(c.first_name, ' ', c.last_name) 
AS customer_name, SUM(p.amount) AS total_revenue FROM customer c 
...
-- Q.9 Utilize a CTE with a window function to rank films based on their rental duration from the tables
...
-- CTE and Filtering:
-- Q.10 Create a CTE to list customers who have made more than two rentals, and then join this CTE with the customer -- table to retrieve additional customer details
with customerrentals as( SELECT customer_id , count(rental_id) as rental_count from rental
GROUP BY customer_id HAVING (rental_count)>2 ) select c.*,rental_count FROM customer c join customerrentals on c.customer_id =customerrentals.customer_id 
...
GROUP BY rental_month ) SELECT rental_month ,total_rental FROM Monthly_rentals ORDER BY rental_month ;

-- CTE for Pivot Operations:
-- Q.12 Use a CTE to pivot the data from the payment table to display the total payments made by each customer in -- separate columns for different payment methods
...
with customer_payments as(select c.customer_id , sum(p.amount) as payment_amount from payment p join customer c on c.customer_id = p. customer_id group by customer_id )
...
SELECT actor1_id, actor2_id, COUNT(*) AS films_together
FROM ActorPairs
GROUP BY actor1_id, actor2_id
ORDER BY films_together DESC;

-- CTE for Recursive Search:
-- Q.14) Implement a recursive CTE to find all employees in the staff table who report to a specific manager, considering the reports_to column.
WITH RECURSIVE DirectReports AS (
    SELECT employee_id, employee_name, reports_to
    FROM staff
    WHERE reports_to = manager_id -- Specify the manager's ID here
UNION ALL
    SELECT s.employee_id, s.employee_name, s.reports_to
    FROM staff s
    INNER JOIN DirectReports d ON s.reports_to = d.employee_id
)
